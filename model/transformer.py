...
#  def __init__(self, ntoken: int, d_model: int, nhead: int, d_hid: int,
#                  nlayers: int, dropout: float = 0.1, random_attention_ratio: float = 0.05):
#    def __init__(self, ntoken: int, d_model: int, nhead: int, d_hid: int,
#                nlayers: int, dropout: float = 0.1, local_attention_window: int = 50):
        super().__init__()
        self.model_type = 'Transformer'
        self.pos_encoder = PositionalEncoding(d_model, dropout)
        #TransformerEncoderLayer封装了多头注意力
        encoder_layers = TransformerEncoderLayer(d_model, nhead, d_hid, dropout, batch_first=True)
        #encoder_layers = RandomAttentionTransformerEncoderLayer(d_model, nhead, d_hid, dropout, random_attention_ratio)
        #encoder_layers = LocalAttentionTransformerEncoderLayer(d_model, nhead, d_hid, dropout, local_attention_window)
