...
class FineTune(object):
    ...
    def train(self):
        ...
        for epoch_counter in range(self.config['epochs']):
            ...
            if epoch_counter % self.config['eval_every_n_epochs'] == 0:
                ...
                self.val_maes.append(valid_mae)
        ...
        plt.subplot(1, 2, 2)
        plt.plot(self.epochs, self.val_maes, label="Val MAE", color="orange")
        plt.xlabel("Epoch")
        plt.ylabel("MAE")
        plt.title("Validation MAE")
        plt.tight_layout()
        plt.savefig(os.path.join(self.writer.log_dir, "training_curve.png"), dpi=300)
        plt.close()
    ...
    def _load_pre_trained_weights(self, model):
        try:
            ...
            load_state = torch.load(os.path.join(checkpoints_folder, 'model_transformer_1.pth'),  map_location=self.config['gpu']) 
...
if __name__ == "__main__":
    ...
    log_dir = os.path.join(
        'training_results/finetuning/Transformer_LSTM_Conv',
